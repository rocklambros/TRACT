...
**Bootstrap CI** — A confidence interval computed by resampling the evaluation data with replacement (typically 10,000 times) and computing the metric on each resample. TRACT reports 95% bootstrap confidence intervals for all evaluation metrics. Example: "hit@1 = 0.5918 [0.5170, 0.6667]" means the true hit@1 is between 0.5170 and 0.6667 with 95% confidence. (This worked example used to quote 0.537 [0.463, 0.612] -- a withdrawn figure whose interval PRD.md section 6.4 describes as "arithmetic on the point estimate rather than the bootstrap it claimed to be", which made it a poor illustration of a bootstrap CI.)
...
**hit@k** — The fraction of controls for which the correct CRE hub appears in the model's top-k predictions. hit@1 measures exact accuracy; hit@5 measures whether the correct hub is among the top 5 candidates. TRACT's most recent measured result is Campaign 2's held-out AI test round: trained hit@1 = 0.5918 against a paired zero-shot baseline of 0.4558, n=147. See `docs/campaign2-results.md` for what that number does and does not license. The figure 0.537 that stood here is WITHDRAWN (PRD.md section 6.4).
