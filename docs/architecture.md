...
> **Withdrawn 2026-08-15.** Every figure in this section is retained as the
> record of what campaign 1 measured. None of it is a current result and none of
> it should be quoted. `results/phase1b/lofo_title_only/aggregate_metrics.json`
> records `point_estimate_pass: true` beside `ci_low_pass: false` and
> `verdicts_agree: false` — the delta interval is [0.041, 0.218] against a
> pre-registered 0.10 threshold, so only the point estimate ever cleared the
> bar. The arm was the best of four configurations with no correction for that
> selection; familywise-adjusted the lower bound is 0.020. On the 1,265-control
> validation roster fine-tuning delivers macro delta -0.0004 at p = 0.98. The
> full withdrawal is PRD.md §6.4.
...
As printed above, OWASP AI Exchange scored highest (76.2% hit@1) and MITRE ATLAS lowest (27.9%). Why the folds ranked that way was never measured — the explanations that stood here (control scoping, technique granularity) were conjecture stated as cause.

The two tables above were also never reconciled with the run they describe. `results/phase1b/lofo_title_only/aggregate_metrics.json` records aggregate hit@1 0.531, MITRE ATLAS 0.302 and OWASP AI Exchange 0.714 — none of them the numbers printed here. Gate 1 was recorded as a pass on the point estimate alone, and the "all folds non-negative" reading that once closed this section was read off per-fold deltas that the recorded run does not reproduce.
...
- **MITRE ATLAS difficulty** — ATLAS was the weakest fold of the withdrawn campaign-1 run (27.9% hit@1; see the erratum in §7 before quoting that figure). Many ATLAS techniques are novel concepts that straddle multiple CRE categories, but no measurement establishes that as the cause rather than model failure.
